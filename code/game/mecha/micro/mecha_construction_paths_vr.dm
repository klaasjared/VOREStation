...
////////////Jones Mecha Construction////////////

/datum/construction/mecha/jones_chassis
	steps = list(list("key"=/obj/item/mecha_parts/part/jones_torso),//1
					 list("key"=/obj/item/mecha_parts/part/jones_left_arm),//2
					 list("key"=/obj/item/mecha_parts/part/jones_right_arm),//3
					 list("key"=/obj/item/mecha_parts/part/jones_left_leg),//4
					 list("key"=/obj/item/mecha_parts/part/jones_right_leg)//5
					)

/datum/construction/mecha/jones_chassis/custom_action(step, obj/item/I, mob/user)
...
/datum/construction/mecha/ripley_chassis/spawn_result()
	...
	const_holder.icon = 'icons/mecha/mech_construction_vr.dmi'
	const_holder.icon_state = "jones0"
	const_holder.density = 1
	const_holder.overlays.len = 0
	spawn()
		qdel(src)
...
/datum/construction/reversible/mecha/janimech
	result = "/obj/mecha/working/janimech"
	steps = list(
					//1
					list("key"=/obj/item/weapon/weldingtool,
							"backkey"=IS_WRENCH,
...
/datum/construction/reversible/mecha/ripley/custom_action(index, diff, obj/item/I, mob/user)
	...
	switch(index)
		if(14)
			user.visible_message("[user] connects [holder] hydraulic systems", "You connect [holder] hydraulic systems.")
			holder.icon_state = "jones1"
		if(13)
			if(diff==FORWARD)
				user.visible_message("[user] activates [holder] hydraulic systems.", "You activate [holder] hydraulic systems.")
				holder.icon_state = "jones2"
			else
				user.visible_message("[user] disconnects [holder] hydraulic systems", "You disconnect [holder] hydraulic systems.")
				holder.icon_state = "jones0"
		if(12)
			if(diff==FORWARD)
				user.visible_message("[user] adds the wiring to [holder].", "You add the wiring to [holder].")
				holder.icon_state = "jones3"
			else
				user.visible_message("[user] deactivates [holder] hydraulic systems.", "You deactivate [holder] hydraulic systems.")
				holder.icon_state = "jones1"
		if(11)
...
/datum/construction/reversible/mecha/jones/spawn_result()
	..()
	feedback_inc("mecha_jones_created",1)
	return
